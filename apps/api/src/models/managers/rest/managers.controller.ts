...
export class ManagersController {
  ...
  create(@Body() createManagerDto: CreateManager, @GetUser() user: GetUserType) {
    checkRowLevelPermission(user, (createManagerDto as any).uid)
    return this.prisma.manager.create({ data: createManagerDto as any })
  }
  ...
  async update(
    ...
    checkRowLevelPermission(user, (manager as any)?.uid)
    return this.prisma.manager.update({
      where: { uid },
      data: updateManagerDto as any,
    })
  }
