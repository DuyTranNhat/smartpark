...
export class ValetsController {
  ...
  create(@Body() createValetDto: CreateValet, @GetUser() user: GetUserType) {
    checkRowLevelPermission(user, (createValetDto as any).uid)
    return this.prisma.valet.create({ data: createValetDto as any })
  }
  ...
  async update(
    ...
    checkRowLevelPermission(user, (valet as any)?.uid)
    return this.prisma.valet.update({
      where: { uid },
      data: updateValetDto as any,
    })
  }
