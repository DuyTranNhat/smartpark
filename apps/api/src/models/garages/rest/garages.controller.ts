...
export class GaragesController {
  ...
  create(@Body() createGarageDto: CreateGarage, @GetUser() user: GetUserType) {
    checkRowLevelPermission(user, (createGarageDto as any).uid)
    return this.prisma.garage.create({ data: createGarageDto as any })
  }
  ...
  async update(
    ...
    checkRowLevelPermission(user, (garage as any)?.uid)
    return this.prisma.garage.update({
      where: { id },
      data: updateGarageDto as any,
    })
  }
